...
#ifndef MODE_DEBUG
#define CONF_FILE_P(APP) "/etc/controller/"#APP"m1.conf"
...
#define SELECT_PROG_SEM_P(APP) "select "#APP".sem_mapping.prog_sem_id, "#APP".sem_m1.sensor_id, "#APP".sem_m1.sensor_id_ambient, "#APP".sem_m1.em_id_heater, "#APP".sem_m1.em_id_cooler,  "#APP".sem_m1.doff from "#APP".sem_mapping, "#APP".sem_m1 where "#APP".sem_mapping.sem_id="#APP".sem_m1.id and "#APP".sem_mapping.prog_id=%d"
...
typedef struct {
    int id;
    Sensor *sensor;
    Sensor *sensor_ambient;
    EM *em_heater;
    EM *em_cooler;
    float doff;//if ambient-doff <= x >=ambient+doff then no regulation
} ProgSEM;
