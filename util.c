...
int checkStep(Step *item) {
     if (item->sem == NULL) {
        fprintf(stderr, "ERROR: checkStep: no SEM attached to step with id = %d\n", item->id);
        return 0;
    }
    if (item->sem->em_heater == NULL) {
        fprintf(stderr, "ERROR: checkStep: no heater EM attached to step with id = %d\n", item->id);
        return 0;
    }
    if (item->sem->em_cooler == NULL) {
        fprintf(stderr, "ERROR: checkStep: no cooler EM attached to step with id = %d\n", item->id);
        return 0;
    }
    if (item->sem->sensor_ambient == NULL) {
        fprintf(stderr, "ERROR: checkStep: no ambient sensor attached to step with id = %d\n", item->id);
        return 0;
    }
    if (item->sem->sensor == NULL) {
        fprintf(stderr, "ERROR: checkStep: no sensor attached to step with id = %d\n", item->id);
        return 0;
    }
    switch (item->pid_heater->mode) {
        case PID_MODE_HEATER:
        case PID_MODE_COOLER:
            break;
        default:
            fprintf(stderr, "ERROR: checkStep: bad heater PID mode for step with id = %d\n", item->id);
            return 0;
    }
    switch (item->pid_cooler->mode) {
        case PID_MODE_HEATER:
        case PID_MODE_COOLER:
            break;
        default:
            fprintf(stderr, "ERROR: checkStep: bad cooler PID mode for step with id = %d\n", item->id);
            return 0;
    }
...
PID *getProgPIDById(int id, char mode, const Prog *prog) {
    size_t i;
    for (i = 0; i < prog->pid_list.length; i++) {
        if (prog->pid_list.item[i].id == id && prog->pid_list.item[i].pid.mode == mode) {
...
ProgSEM *getProgSEMById(int id, const Prog *prog) {
    size_t i;
    for (i = 0; i < prog->sem_list.length; i++) {
        if (prog->sem_list.item[i].id == id) {
            return &prog->sem_list.item[i];
        }
    }
#ifdef MODE_DEBUG
...
int getStepByIdFdb(int id, const Prog *prog, Step *item) {
    ...
    item->pid_heater = getProgPIDById(atoi(PQgetvalue(r, 0, 1)), PID_MODE_HEATER, prog);
    item->pid_cooler = getProgPIDById(atoi(PQgetvalue(r, 0, 1)), PID_MODE_COOLER, prog);
    item->sem = getProgSEMById(atoi(PQgetvalue(r, 0, 2)), prog);
...
int checkProgPID(const ProgPIDList *list) {
    ...
    for (i = 0; i < list->length; i++) {
        for (j = i + 1; j < list->length; j++) {
            if (list->item[i].id == list->item[j].id && list->item[i].pid.mode == list->item[j].pid.mode) {
                fprintf(stderr, "ERROR: checkProgPID: id and mode are not unique where id = %d\n", list->item[i].id);
                return 0;
            }
        }
    }
...
int checkProgSEM(const ProgSEMList *list) {
    size_t i, j;
    for (i = 0; i < list->length; i++) {
        if (list->item[i].em_heater == NULL) {
            fprintf(stderr, "ERROR: checkProgSEM: no heater EM where id = %d\n", list->item[i].id);
            return 0;
        }
        if (list->item[i].em_cooler == NULL) {
            fprintf(stderr, "ERROR: checkProgSEM: no cooler EM where id = %d\n", list->item[i].id);
            return 0;
        }
        if (list->item[i].sensor_ambient == NULL) {
            fprintf(stderr, "ERROR: checkProgSEM: no ambient sensor where id = %d\n", list->item[i].id);
            return 0;
        }
...
ProgSEMList getProgSEMListFdb(int prog_id, const SensorList *slist, const EMList *elist) {
    ...
    if (list.length > 0) {
        ...
        for (i = 0; i < list.length; i++) {
            ...
            list.item[i].sensor_ambient = getSensorById(atoi(PQgetvalue(r, i, 2)), &sensor_list);
            list.item[i].em_heater = getEMById(atoi(PQgetvalue(r, i, 3)), &em_list);
            list.item[i].em_cooler = getEMById(atoi(PQgetvalue(r, i, 4)), &em_list);
            list.item[i].doff = atof(PQgetvalue(r, i, 5));
        }
    }
    PQclear(r);
...
Prog * getProgByIdFdb(int id, const SensorList *slist, const EMList *elist) {
    ...
    item->sem_list = getProgSEMListFdb(id, slist, elist);
    PQclear(r);
    INIT_PROG_FIELDS
...
void printAll(ProgList *list, PeerList *pl, EMList *el, SensorList *sl) {
    ...
    sendStr("|                   Program SEM                 |\n", &crc);
    sendStr("+-----------+-----------+-----------+-----------+-----------+-----------+-----------+\n", &crc);
    sendStr("|  prog_id  |prog_sem_id| em_heater | em_cooler |   sensor  | sensor_am |    doff   |\n", &crc);
    sendStr("+-----------+-----------+-----------+-----------+-----------+-----------+-----------+\n", &crc);
    PROG_LIST_LOOP_ST
    for (i = 0; i < curr->sem_list.length; i++) {
        snprintf(q, sizeof q, "|%11d|%11d|%11d|%11d|%11d|%11d|%11.2f|\n",
                curr->id,
                curr->sem_list.item[i].id,
                curr->sem_list.item[i].em_heater->id,
                curr->sem_list.item[i].em_cooler->id,
                curr->sem_list.item[i].sensor->id,
                curr->sem_list.item[i].sensor_ambient->id,
                curr->sem_list.item[i].doff
                );
        sendStr(q, &crc);
    }
    PROG_LIST_LOOP_SP
    ...
    sendStr("+-----------+-----------+-----------+-----------+-----------+-----------+-----------+-----------+-----------+\n", &crc);
    sendStr("|  prog_id  |   kind    |     id    |  sensor   | em_heater |   goal    |   dur sec |even_change| stop_kind |\n", &crc);
    sendStr("+-----------+-----------+-----------+-----------+-----------+-----------+-----------+-----------+-----------+\n", &crc);
    PROG_LIST_LOOP_ST
    ...
    snprintf(q, sizeof q, "|%11d|%11s|%11d|%11p|%11p|%11f|%11ld|%11d|%11s|\n",
            ...
            "CURR",
            curr->step_curr.id,
            curr->step_curr.sem->sensor,
            curr->step_curr.sem->em_heater,
            curr->step_curr.goal,
...
void printProgSEMList(ProgSEMList *list) {
    ...
    puts("SEM list");
    puts("+-----------+-----------+-----------+-----------+-----------+-----------+-----------+-----------+-----------+");
    puts("|   id      |   s_id    |   s_rid   |  sa_id    |  sa_rid   |  emh_id   | emh_rid   |  emc_id   | emc_rid   |");
    puts("+-----------+-----------+-----------+-----------+-----------+-----------+-----------+-----------+-----------+");
    for (i = 0; i < list->length; i++) {
        printf("|%11d|%11d|%11d|%11d|%11d|%11d|%11d|%11d|%11d|\n",
                list->item[i].id,
                list->item[i].sensor->id,
                list->item[i].sensor->remote_id,
                list->item[i].sensor_ambient->id,
                list->item[i].sensor_ambient->remote_id,
                list->item[i].em_heater->id,
                list->item[i].em_heater->remote_id,
                list->item[i].em_cooler->id,
                list->item[i].em_cooler->remote_id
                );
    }
    puts("+-----------+-----------+-----------+-----------+-----------+-----------+-----------+-----------+-----------+");
}
