...
(
  ...
  ki real DEFAULT 100,
  kd real DEFAULT 100,
  CONSTRAINT pid_pkey PRIMARY KEY (id, mode),
  CONSTRAINT pid_mode_check CHECK (mode = 'h'::bpchar OR mode = 'c'::bpchar)
)
WITH (
  OIDS=FALSE
);
CREATE TABLE regstp.sem_m1
(
  id integer NOT NULL,
  sensor_id_ambient integer NOT NULL  DEFAULT -1,
  sensor_id integer NOT NULL  DEFAULT -1,
  em_id_heater integer NOT NULL  DEFAULT -1,
  em_id_cooler integer NOT NULL  DEFAULT -1,
  doff real DEFAULT 0.25,
  CONSTRAINT sem_m1_pkey PRIMARY KEY (id)
)
WITH (
  OIDS=FALSE
);
