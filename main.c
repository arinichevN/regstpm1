...
#ifdef MODE_DEBUG
        fputs("ERROR: sensorRead: response: temperature sensor state is bad\n", stderr);
#endif
        s->value.state = 0;
        return 0;
    }
            ...
            switch (buf_in[0]) {
                case ACP_QUANTIFIER_BROADCAST:
                {
                    if (lockProgAll()) {
                        if (lockProgList()) {
                            PROG_LIST_LOOP_ST
                            controlEM(curr->step_curr.sem->em_heater, 0.0f);
                            controlEM(curr->step_curr.sem->em_cooler, 0.0f);
                            PROG_LIST_LOOP_SP
                            freeProg(&prog_list);
                            unlockProgList();
                        }
                ...
                case ACP_QUANTIFIER_SPECIFIC:
                    if (lockProgList()) {
                        for (i = 0; i < i1l.length; i++) {
                            ...
                            if (p != NULL) {
                                controlEM(p->step_curr.sem->em_heater, 0.0f);
                                controlEM(p->step_curr.sem->em_cooler, 0.0f);
                            }
...
#define VALUE_AMBIENT item->sem->sensor_ambient->value.value
#define VALUE item->sem->sensor->value.value
#define DOFF item->sem->doff
#define HEATER_OFF controlEM(item->sem->em_heater, 0.0f);
#define COOLER_OFF controlEM(item->sem->em_cooler, 0.0f);
#define EM_OFF HEATER_OFF COOLER_OFF
#define STEP_OFF  item->state = OFF;EM_OFF
...
int stepControl(Step *item) {
    ...
    switch (item->state) {
        case INIT:
            item->pid_heater->reset = 1;
            item->pid_cooler->reset = 1;
            item->tmr.ready = 0;
            ...
            break;
        case RUN:
            if (sensorRead(item->sem->sensor) && sensorRead(item->sem->sensor_ambient)) {
                if (item->cgoal > VALUE_AMBIENT + DOFF) {//heater
                    item->output = pid(item->pid_heater, item->cgoal, VALUE);
                    COOLER_OFF
                    controlEM(item->sem->em_heater, item->output);
#ifdef MODE_DEBUG
                    printf("goal=%.1f real=%.1f ambient=%.1f pid_mode=heater out=%.1f\n", item->cgoal, VALUE, item->sem->sensor_ambient->value.value, item->output);
                    //     printf("pid_mode=%c pid_ie=%.1f\n", item->pid->mode, item->pid->integral_error);
#endif
                } else if (item->cgoal < VALUE_AMBIENT - DOFF) {//cooler
                    item->output = pid(item->pid_cooler, item->cgoal, VALUE);
                    HEATER_OFF
                    controlEM(item->sem->em_cooler, item->output);
#ifdef MODE_DEBUG
                    printf("goal=%.1f real=%.1f ambient=%.1f pid_mode=cooler out=%.1f\n", item->cgoal, VALUE, item->sem->sensor_ambient->value.value, item->output);
                    //     printf("pid_mode=%c pid_ie=%.1f\n", item->pid->mode, item->pid->integral_error);
#endif
                } else {
                    EM_OFF
#ifdef MODE_DEBUG
                            printf("goal=%.1f real=%.1f ambient=%.1f pid_mode=off out=%.1f\n", item->cgoal, VALUE, item->sem->sensor_ambient->value.value, item->output);
                    //     printf("pid_mode=%c pid_ie=%.1f\n", item->pid->mode, item->pid->integral_error);
#endif
                }
                //goal correction
                switch (item->state_ch) {
                    case INIT:
                        if (item->even_change && item->stop_kind == STOP_KIND_TIME) {
                            if (item->sem->sensor->value.state) {
                                item->goal_correction = 0;
                                item->temp_start = VALUE;
